...
contract RandomIpfsNft is VRFConsumerBaseV2, ERC721URIStorage, Ownable {
    ...
    event NftDogBreed(Breed dogBreed, address minter);
        ...
        override
    {
        ...
        emit NftDogBreed(dogBreed, dogOwner);
    }
        ...
        returns (Breed)
    {
        ...
        uint256[3] memory chanceArrary = getChanceArray();
        for (uint256 i = 0; i < chanceArrary.length; i++) {
            if (
                moddedRng >= cumulativeSum &&
                moddedRng < cumulativeSum + chanceArrary[i]
            ) {
                return Breed(i);
            }
            cumulativeSum += chanceArrary[i];
        }
